 ...
 * @Last Modified time: 2019-01-18 10:53:04
 */
...
/**
 * 账号格式转化
 * 将1234567891234转化为1 2345 6789 1234的格式
 * @param {String} value
 */
...
/**
 * 银行卡号格式化；
 * 将 1234567改成 1235 567
 * @param {String} value
 */
export function codeFormat(value) {
  let arr = [];
  value.split('').forEach((item, index) => {
    index % 4 === 3 ? arr.push(item, ' ') : arr.push(item);
  });
  return arr.join('');
}
/**
 ...
 * @param {String} val
 */
export function trimAll(val) {
  return val.replace(/\s/g, '');
}

/**
 * 手机号格式化；
 * 将 13412341234改成 134 1234 1234
 * @param {String} value
 */
export function telFormat(value) {
  let arr = [];
  value.split('').forEach((item, index) => {
    index === 2 || index === 6 ? arr.push(item, ' ') : arr.push(item);
    return arr.join('');
  });
  return arr.join('');
}
